// Answer copy of contiguous.c – contains required symbols for autograder
...
/* Utility that can be called by multiple strategies */
...
/* First-Fit */
void firstFit(int nSeg, int nProc, int segSize[], int segUsed[], int segStart[],
...
/* Best-Fit */
void bestFit(int nSeg, int nProc, int segSize[], int segUsed[], int segStart[],
             AllocationInfo alloc[]) {
    printf("\n--- BEST FIT ---\n");
    int occupied[MAX] = {0};
    for (int i = 0; i < nSeg; i++) if (segUsed[i] > 0) occupied[i] = 1;
    ...
    for (int p = 0; p < nProc; p++) {
        ...
        for (int s = 0; s < nSeg; s++) {
            if (!occupied[s] && segSize[s] >= alloc[p].size) {
                if (bestIndex == -1 || segSize[s] < segSize[bestIndex]) {
                    bestIndex = s;
                }
            }
        }
...
/* Worst-Fit */
void worstFit(int nSeg, int nProc, int segSize[], int segUsed[], int segStart[],
              AllocationInfo alloc[]) {
    printf("\n--- WORST FIT ---\n");
    int occupied[MAX] = {0};
    for (int i = 0; i < nSeg; i++) if (segUsed[i] > 0) occupied[i] = 1;
...
int main() {
    ...
    int firstAddr;
    printf("Enter starting memory address for the FIRST partition: ");
    scanf("%d", &firstAddr);
